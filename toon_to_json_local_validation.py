...
# ==============================================================================
# TOON GENERATION PROMPT (same as above)
# ==============================================================================
...
def are_equivalent(json_a_str, json_b_str):
    ...
    try:
        a = json.loads(json_a_str)
        b = json.loads(json_b_str)
        return a == b
    except json.JSONDecodeError:
        try:
            # Try to fix JSON_A
            fixed_a = fix_truncated_json(json_a_str)
            a = json.loads(fixed_a)
            b = json.loads(json_b_str)
            return a == b
        except:
            return False
...
def run_test_case(data_desc, test_name="Test"):
    ...
    # --- LOCAL VALIDATION ---
    is_equivalent = are_equivalent(json_A, json_B)
    verdict = "YES" if is_equivalent else "NO"

    print("\n" + "="*90)
    print(f"✅ VALIDATION (LOCAL): {verdict}")
    print("="*90)
